...
class DriverDB:
    ...
    def __init__(self, use_cache=False):
        ...
        if self.use_cache:
            ...
            try:
                self.cache = cPickle.load(open(self.cache_path, 'rb'))
            except (cPickle.PickleError, IOError), e:
...
class XMLRPCDriverDB(DriverDB):
    ...
    def _do_update(self, hwids):
        ...
        client = xmlrpclib.ServerProxy(self.url)
    ...
    def _do_update(self, hwids):
        ...
        def _ld_callback(status, drv_list, data):
            ...
            for driver, info in data.iteritems():
                ...
                # needed to disambiguate e. g. splix and splix2
                if driver != did.properties['package']:
                    did.properties['version'] = driver.replace('driver/', '')
        ...
        for hwid in hwids:
            ...
            t = op.listDrivers(hwid.id, _ld_callback,
                ...
                extra_options={'packagesystem': opo_pkgsystem, 'architectures': 'noarch'})
            threads.append(t)
...
def _get_printers():
    ...
    if _get_printers.cache is not None:
        return _get_printers.cache

    _get_printers.cache = set()
...
def _driverid_to_handler(did, backend, mode):
    ...
    for a in ('driver_type', 'jockey_handler', 'version', 'description',
        'long_description', 'repository', 'package', 'driver_vendor', 'free',
        'license', 'recommended'):
        try:
            del kwargs[a]
        except KeyError:
            ...
            if did['kernel_module'] not in _driverid_to_handler.ignored:
                ...
                if not get_modinfo(did['kernel_module']):
                    if 'free' not in did.properties or \
                       ...
                       logging.warning('DriverID for module %s does not refer '
                           'to a locally available module and does not specify '
                           'free/description/package; ignoring', did['kernel_module'])
                       return None
    ...
    for hwid in hardware:
        ...
        if len(dids) == 1:
            try:
                ...
                logging.debug('DriverID %s for %s is the only alternative, removed recommendation', str(dids[0].properties), hwid)
            except KeyError:
                pass
