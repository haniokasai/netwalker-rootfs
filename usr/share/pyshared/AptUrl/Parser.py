...
def match_against_whitelist(raw_url):
    ...
    for char in raw_url:
        if not char in whitelist:
            raise InvalidUrlException, _("Non whitelist char in the uri")
    return True
...
def parse(full_url, mapping=apturl_substitution_mapping):
    ...
    for url in full_url.split(";"):
        if not ":" in url:
            raise InvalidUrlException, _("No ':' in the uri")
        ...
        for package in packages:
            ...
            if "?" in package:
                ...
                for s in key_value_pairs:
                    ...
                    try:
                        if ' ' in value:
                            raise InvalidUrlException, _("Whitespace in key=value")
                        if type(getattr(apt_url, key)) == type([]):
                            getattr(apt_url, key).append(value)
                        else:
                            setattr(apt_url, key, value)
                    except Exception, e:
                        raise InvalidUrlException, _("Exception '%s'") % e
            ...
            if not is_format_package_name(apt_url.package):
                raise InvalidUrlException, "Invalid package name '%s'" % apt_url.package
