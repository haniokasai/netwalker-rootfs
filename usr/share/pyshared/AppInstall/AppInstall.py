...
from Progress import GtkOpProgressWindow, GtkCdromProgress
from Util import *
...
class AppInstall(SimpleGladeApp):
    ...
    def __init__(self, options, activation_style):
        ...
        SimpleGladeApp.__init__(self, domain="gnome-app-install",
                                path=options.datadir+"/gnome-app-install.glade")
        ...
        self.packageWorker = PackageWorker(options.addon_cd)
    ...
    def addCD(self):
        """ check for addon cd and if available and not yet added
            add itadd addon cd
        """
        if self.addon_cd is not None:
            try:
                self.window_main.set_sensitive(False)
                self.setBusy(True)

                cd_desktopdir = os.path.join(self.addon_cd,"app-install")
                if os.path.exists(cd_desktopdir):
                    cdrom = apt.cdrom.Cdrom(progress=GtkCdromProgress(self), mountpoint=self.addon_cd)
                    if not cdrom.inSourcesList:
                        cdrom.add()
                    self.desktopdir=cd_desktopdir
                    self.cachedir=None
            except SystemError, e:
                print "SystemError while adding addon CD:\n%s" % e
    ...
    def _checkAptCache(self):
        ...
        #          files do not change
        time_cache = self.config.get_int("/apps/gnome-app-install/cache_dialog_time")
    ...
    def updateCache(self, filter=SHOW_ONLY_SUPPORTED):
        ...
        progress = GtkOpProgressWindow(self.glade, progress_transient_for)
        try:
    ...
    def applyChanges(self, to_add, to_rm):
        ...
        ret = self.packageWorker.perform_action(self.window_main, self.cache,
                                                pkgs_add, pkgs_rm)
