...
#            self.tzmap.show()
    ...
    def run(self):
        ...
        self.steps.set_current_page(self.steps.page_num(self.step_welcome))
        # TODO cjwatson 2006-07-07: why isn't on_steps_switch_page getting
        # invoked?
        self.set_current_page(self.steps.page_num(self.step_welcome))
        ...
        while self.current_page is not None:
            ...
            elif current_name == 'step_welcome':
                self.dbfilter = welcome.Welcome(self)
            else:
...
#            dbfilter.run_command(auto_process=True)
    ...
    def allow_change_step(self, allowed):
        ...
        self.quit.set_sensitive(allowed)
        ...
        if self.quit.get_property('visible'):
            self.quit.hide()
            self.quit.show()
    ...
    def on_quit_clicked(self, widget):
        response = self.question_dialog(
            self.get_string('end'),
            self.get_string('stop'),
            ('oem-config/text/go_back', 'oem-config/text/yes'))
        if response is None or response == 'oem-config/text/yes':
            self.exit_yes = True
            if self.dbfilter is not None:
                self.dbfilter.cancel_handler()
